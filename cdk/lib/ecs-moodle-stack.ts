...
export class EcsMoodleStack extends cdk.Stack {
  ...
  constructor(scope: cdk.App, id: string, props: EcsMoodleStackProps) {
    ...
    //Moodle ECS Task Definition Green
    // const moodleTaskDefinitionGreen = new ecs.FargateTaskDefinition(this, 'moodle-task-def-Green', {
    //   cpu: 2048,
    //   memoryLimitMiB: 4096
    // });
    // moodleTaskDefinitionGreen.addToExecutionRolePolicy(iam.PolicyStatement.fromJson({
    //   "Effect": "Allow",
    //   "Action": [
    //       "ecr:GetAuthorizationToken",
    //       "ecr:BatchCheckLayerAvailability",
    ...
    // // EFS Volume
    // moodleTaskDefinitionGreen.addVolume({
    //   name: 'moodle',
    //   efsVolumeConfiguration: {
    //     fileSystemId: moodleEfs.fileSystemId,
    //     transitEncryption: "ENABLED",
    ...
    // // Moodle container definition
    // const moodleContainerDefinitionGreen = moodleTaskDefinitionGreen.addContainer('moodle-containerGreen', {
    //   containerName: 'moodle',
    //   image: ecs.ContainerImage.fromRegistry(props.moodleImageUri),
    //   memoryLimitMiB: 4096,
    //   portMappings: [{ containerPort: 8080 }],
    ...
    //   logging: ecs.LogDrivers.awsLogs({ streamPrefix: 'ecs-moodle' })
    // });
    // moodleContainerDefinitionGreen.addMountPoints({
    //   sourceVolume: 'moodle',
    //   containerPath: '/bitnami',
    //   readOnly: false
    // });
    ...
    const cfnEcs = moodleService.node.defaultChild as ecs.CfnService
    cfnEcs.addDeletionOverride('DeploymentConfiguration.Alarms')

        // Moodle ECS Service Green
        // const moodleServiceGreen = new ecs.FargateService(this, 'moodle-service-Green', {
        //   cluster: cluster,
        //   taskDefinition: moodleTaskDefinitionGreen,
        //   desiredCount: props.serviceReplicaDesiredCount,
        //   capacityProviderStrategies: [ // Every 1 task which uses FARGATE, 3 tasks will use FARGATE_SPOT (25% / 75%)
        //     {
        //       capacityProvider: 'FARGATE_SPOT',
    ...
    const httpListener8080 = alb.addListener('http-listener-8080', {
      port: 8080,
      protocol: elbv2.ApplicationProtocol.HTTP,
      open: true,
    })
    ...
    // Target Group Blue
    const targetGroupBlue = httpListener8080.addTargets('moodle-service-tg-blue', {
      port: 443,
      targets: [
    ...
    //Target Group Green
    const targetGroupGreen = httpsListener.addTargets('moodle-service-tg-green', {
      port: 443,
      targets: [
        moodleService.loadBalancerTarget({
          containerName: 'moodle',
